...
class Settings(BaseSettings):
    ...
    # "s3" — два независимых S3: основной (HostKey) + зеркало (Backblaze B2).
    ...
    # Пороги отставания реплики, в секундах. Имеют смысл прежде всего в
    # асинхронном режиме (серверы в разных странах): отставание реплики — это
    # ровно тот объём работы, который пропадёт при внезапной потере главного.
    # Без порогов «асинхронно» незаметно превращается в «реплики нет вторые
    # сутки», и узнают об этом уже после аварии.
