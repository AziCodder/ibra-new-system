...
async def notify_shipment_sent(logistics: Logistics, session: AsyncSession) -> None:
    ...
    Nothing in here may fail the request — the shipment is already saved, and an
    error would push the operator to create it a second time.
    """
    ...
    try:
        ...
        logger.exception("Shipment %s created, but its notification could not be built", logistics.id)
