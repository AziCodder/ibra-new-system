...
|--------|----------|--------|
| 4.9 | PATCH/DELETE /api/orders/{id} (admin only) + count_order_dependencies() + 3 теста написаны. pytest запускался — падает с OSError/WinError 1225 (connection refused на 5433), т.е. чисто инфраструктурная причина, не логическая ошибка. НЕ закрываю — это права доступа/целостность данных (§4.3 требует зелёных тестов реально, не по аналогии) | ⏳ |
