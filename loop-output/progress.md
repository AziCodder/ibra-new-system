...
## Текущая фаза: 4
## Текущая задача: 4.9 — Редактирование и удаление заказа (код+тесты готовы, ждёт зелёных тестов — Docker недоступен 3-й тик подряд)
...
|--------|----------|--------|
| 4.9 | PATCH/DELETE /api/orders/{id} (admin only) + count_order_dependencies() + 3 теста написаны. pytest запускался — падает с OSError/WinError 1225 (connection refused на 5433), т.е. чисто инфраструктурная причина, не логическая ошибка. НЕ закрываю — это права доступа/целостность данных (§4.3 требует зелёных тестов реально, не по аналогии). 12:45 — Docker всё ещё недоступен, 4-й тик подряд (~80 мин); GUI-процесс не перезапускался с 11:29, упал именно бэкенд-движок — похоже на повторяющийся краш, не разовый сбой. Отправил уведомление пользователю | ⏳ |
